...
MainWindow.ToggleKey = Enum.KeyCode.K

local Tabs = {
    Info = MainWindow:Tab({ Title = "Information", Icon = "info", Desc = "DYHUB" }),
    Blade = MainWindow:Divider(),
    Main = MainWindow:Tab({ Title = "Main", Icon = "eye", Desc = "DYHUB" }),
    Misc = MainWindow:Tab({ Title = "Misc", Icon = "sparkles", Desc = "DYHUB" }),
    Fight = MainWindow:Tab({ Title = "Fighting", Icon = "sword", Desc = "DYHUB" }),
    Tech = MainWindow:Tab({ Title = "Tech", Icon = "wrench", Desc = "DYHUB" }),
    Lag = MainWindow:Tab({ Title = "Lag-Ping", Icon = "leaf", Desc = "DYHUB" }),
    Anim = MainWindow:Tab({ Title = "Animations", Icon = "package", Desc = "DYHUB" }),
    Place = MainWindow:Tab({ Title = "Places", Icon = "map", Desc = "DYHUB" }),
    Moveset = MainWindow:Tab({ Title = "Moveset", Icon = "star", Desc = "DYHUB" }),
    Fling = MainWindow:Tab({ Title = "Fling", Icon = "user", Desc = "DYHUB" }),
}
...
Tabs.Main:Button({
    Title = "DYHUB Mini",
    Desc = "First version of the DYHUB but rly usefull",
    Locked = false,
    Callback = function()
...
local function ShowNotification(title, text, icon, duration)
    StarterGui:SetCore("SendNotification", {
        Title = title or "DYHUB",
        Text = text,
        Icon = icon or "rbxassetid://104487529937663",
    ...
    if FlingFlags.notifyOnDeath and SelectedFlingTarget then
        ...
        if hum then
            FlingDeathConn = hum.Died:Connect(function()
                ShowNotification("DYHUB", SelectedFlingTarget.DisplayName .. " Died")
            end)
        end
    end
end
...
local function SelectFlingTarget(name)
    ...
    if not name or name == "" then
        ShowNotification("DYHUB", "No One Selected")
        return
    end
    ...
    if target then
        ...
        ShowNotification("DYHUB", "Selected: " .. target.DisplayName, "https://www.roblox.com/headshot-thumbnail/image?userId=" .. target.UserId .. "&width=420&height=420&format=png", 10)
        UpdateFlingDeathNotify()
    else
        ShowNotification("DYHUB", "Player Not Found...")
    end
end
...
Tabs.Fling:Button({
    ...
    Callback = function()
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") return end
        local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        local tRoot = SelectedFlingTarget.Character and SelectedFlingTarget.Character:FindFirstChild("HumanoidRootPart")
        if myRoot and tRoot then
            myRoot.CFrame = tRoot.CFrame
        else
            ShowNotification("DYHUB", "Could not find character to teleport.")
        end
    end,
})
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.teleport = false return end
        task.spawn(function()
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then return end
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.fling = false return end
        task.spawn(function()
...
Tabs.Fling:Button({
    ...
    Callback = function()
        ...
        if hum then
            ...
            ShowNotification("DYHUB", "Could not find the player's character to view.")
        end
    end,
})
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.view = false return end
        task.spawn(function()
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then return end
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.aimLockCam = false return end
        task.spawn(function()
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then return end
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.aimLockChar = false return end
        task.spawn(function()
...
Tabs.Fling:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then return end
        if not SelectedFlingTarget then ShowNotification("DYHUB", "No One Selected") FlingFlags.orbit = false return end
...
Players.PlayerRemoving:Connect(function(p)
    if SelectedFlingTarget and p == SelectedFlingTarget then
        ShowNotification("DYHUB", p.DisplayName .. " left the game")
        SelectedFlingTarget = nil
